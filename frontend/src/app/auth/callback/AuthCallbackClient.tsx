...
export function AuthCallbackClient() {
  ...
  useEffect(() => {
    ...
    if (oauthError) {
      ...
      setMessage(
        oauthError === 'account_disabled'
          ...
          : 'ورود با گوگل لغو شد یا کامل نشد.',
      );
      return;
    }
    ...
    if (!code) {
      setStatus('error');
      setMessage('کد ورود گوگل دریافت نشد.');
      return;
    }
    ...
    void (async () => {
      try {
        ...
        if (
          ['admin', 'support', 'supervisor'].includes(result.user.role)
        ) {
          window.location.replace(
            new URL(result.redirect_to || '/panel/', DJANGO_BASE_URL).toString(),
          );
        } else {
        ...
        setMessage(
          getApiErrorMessage(
            error,
            'تکمیل ورود با گوگل ناموفق بود. دوباره تلاش کنید.',
          ),
        );
      }
    })();
      ...
      <section className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-xl shadow-slate-200/50">
        ...
        {status === 'error' && (
          <button
            ...
            className="mt-6 w-full rounded-xl bg-violet-600 px-4 py-3 text-sm font-semibold text-white"
          >
            بازگشت به صفحه ورود
          </button>
        )}
