...
class OrderRevisionSaveTests(TestCase):
    def setUp(self):
        User = get_user_model()
        self.client_user = User.objects.create_user(
            username="revision_client", password="ClientPass123!", role="client"
        )
        self.supervisor = User.objects.create_user(
            username="revision_supervisor", password="SupPass123!", role="supervisor"
        )
        self.order = Order.objects.create(
            client=self.client_user,
            title="Revision save test order",
        )

    def test_create_revision_does_not_raise(self):
    ...
    def test_update_existing_revision_does_not_raise(self):
        revision = OrderRevision.objects.create(
            order=self.order,
            requested_by=self.supervisor,
            source=OrderRevision.Source.SUPERVISOR,
        ...
        revision.note = "Updated note."
        revision.save()
        revision.refresh_from_db()
        self.assertEqual(revision.note, "Updated note.")
