...
from .workflow import transition_order
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        tags=["Order Workflow"],
        summary="Unified order timeline",
        description="Returns workflow summary and normalized status/activity events.",
    )
    ...
    def timeline(self, request, pk=None):
        ...
        return Response(
            {
                "workflow": get_order_workflow_summary(order),
                "events": build_order_timeline(order),
            },
            status=status.HTTP_200_OK,
        )
    ...
    def start_work(self, request, pk=None):
        ...
        self._update_order_status(
            ...
            new_status=Order.Status.IN_PROGRESS,
            user=request.user,
            note="Editor started work on the order.",
        )
    ...
    def assign_editor(self, request, pk=None):
        ...
        self._update_order_status(
            ...
            user=request.user,
            note="Editor assigned to the order.",
            extra_updates={"editor": editor},
        )
    ...
    def supervisor_approve(self, request, pk=None):
        ...
        if serializer.is_valid():
            rating, created = self._save_or_update_rating(
                order=order,
                user=request.user,
                source=OrderRating.Source.SUPERVISOR,
                serializer=serializer,
            ...
            self._update_order_status(
                ...
                new_status=Order.Status.CLIENT_REVIEW,
                user=request.user,
                note="Supervisor approved delivery for client review.",
                extra_updates={"supervisor_approved_at": timezone.now()},
            )
    ...
    def start_review(self, request, pk=None):
        ...
        self._update_order_status(
            order=order,
            new_status=Order.Status.IN_REVIEW,
            user=request.user,
            note="Order moved to review stage.",
        )
    ...
    def submit(self, request, pk=None):
        ...
        self._update_order_status(
            ...
            user=request.user,
            note="Order submitted for review.",
        )
