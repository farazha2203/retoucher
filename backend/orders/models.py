...
class WorkflowDeadline(models.Model):
    class Status(models.TextChoices):
        ACTIVE = "active", "Active"
        MET = "met", "Met"
    ...
    order = models.ForeignKey(
        Order, on_delete=models.CASCADE, related_name="workflow_deadlines",
        null=True, blank=True,
    )
    project_request = models.ForeignKey(
        "projects.ProjectRequest", on_delete=models.CASCADE,
        related_name="workflow_deadlines", null=True, blank=True,
    )
    ...
    timeout_action = models.CharField(max_length=64, default="notify")
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=(
                    (models.Q(order__isnull=False) & models.Q(project_request__isnull=True))
                    | (models.Q(order__isnull=True) & models.Q(project_request__isnull=False))
                ),
                name="workflow_deadline_exactly_one_target",
            ),
        ]
        indexes = [
            models.Index(fields=["status", "due_at"]),
            models.Index(fields=["order", "status"]),
            models.Index(fields=["project_request", "status"]),
        ]

    def __str__(self):
        target = f"order:{self.order_id}" if self.order_id else f"project:{self.project_request_id}"
        return f"{target} {self.stage} @ {self.due_at}"
